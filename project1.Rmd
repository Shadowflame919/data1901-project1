...
There are various ways to measure the value of a particular college major in achieving employment, such as comparing the number of jobs which require the degree, compared to those that don't.
Another, more direct measure of a majors ability to gain employment is to understand the amount of people left unemployed after they have received their major, i.e. the unemployment rate. 
...
```
This second metric for unemployment could be viewed as essentially rating the 'usefulness' of a degree. 
Even though the plot is attempting to answer the same question, categories like IACS seem to reveal conflicting results.
...
plot_ly(
  ...
  xaxis = list(title = "Usefulness of Requiring Degree (%)")
)
```
