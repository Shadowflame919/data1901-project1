...
Ultimately, however, the popularity of courses appears not to be inherently linked to their employability prospects or expected incomes. This implies that, whether for better or worse, instead of basing choices on these factors, most students may instead be following influencers such as personal preference, external pressure, or availabilities. 
...
```
A histogram of employment rate also shows a fairly normal distribution of popularity. <!-- I don't think this next part is really relevant to the question? --> Interestingly, engineering holds not only the highest median income major, but also the major with the highest employment rate of graduates. However, employment rates of engineering majors are also observed to have much more of a spread. As show below, the employment rate of engineering majors at worst is 2.75 deviations from the mean.
```{r}
...
From this data, it appears that popularity in the courses is not largely influenced by their employability prospects or expected incomes. Instead, both income and employment seem to follow a relatively normal distribution curve. This is in line with what would be expected if the major choices were randomly distributed, implying that a large range of hidden factors will be influencing their decisions. Ultimately, it can be concluded that the choices for majors lie not only in these two factors, but also potentially in others such as enjoyment, external pressure, what the university offers, and more. Further research would have to be done to investigate the impact of these factors. 
